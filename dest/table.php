...
if($sortby === "dues")
{
    $mysql = "SELECT * FROM `destributers` ORDER BY $sortby DESC";
}
else
{
    $mysql = "SELECT * FROM `destributers` ORDER BY DATEDIFF ('$today', `date`) DESC";
}
...
if($sql)
{
    if(mysqli_num_rows($sql)>0)
    {
        ...
        while($row = mysqli_fetch_assoc($sql))
        {
            ...
            $rows .= '<tr onclick="location.href=\'transactions.php?destId=dest_'.$row['id'].'\'">
            <td>'.$slno.'</td>
            <td>'.$row['name'].'</td>
            <td>'.$row['dues'].'</td>
            <td>'.$day.'</td>
